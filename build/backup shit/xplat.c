...
int unwrap_file_out_of_neo_aa(const char *inputBuffer, const char *outputPath, char *pathString, size_t bufferSize) {
...
int extract_contact_signed_shortcut(const char *signedShortcutPath, const char *destPath) {
    extract_signed_shortcut(signedShortcutPath, destPath);
}